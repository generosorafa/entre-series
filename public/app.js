...
function paint(){
 const seconds=Math.ceil(timer.remaining/1000);$('#time').textContent=format(timer.remaining);document.title=timer.state==='running'?format(timer.remaining)+' · Xamai 3S':'Xamai 3S — Seu descanso';
...
function notify(){if(!registration||!('Notification'in window)||Notification.permission!=='granted'||remoteEnabled())return;void registration.showNotification('Xamai 3S',{body:'Descanso concluído. Hora da próxima série.',icon:'./icons/icon-192.png',tag:'xamai-rest',data:{url:new URL('./',location.href).href}}).catch(()=>{})}
...
if(standalone()){$('#install').hidden=true;$('#install-help').textContent='O Xamai 3S já está aberto como aplicativo.'}
...
$('#notifications').addEventListener('click',async()=>{
 ...
 else{await registration.showNotification('Xamai 3S',{body:'Aviso visual ativado. Nesta versão, depende do site em execução.',icon:'./icons/icon-192.png',tag:'xamai-test'});$('#notification-help').textContent='Aviso visual ativado. O agendamento com o site fechado aguarda a configuração do serviço gratuito.'}
